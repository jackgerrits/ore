...
size_t model_part::get_index_count() const { return indexCount; }

GLuint model_part::get_vao_id() const { return vaoID; }

GLuint model_part::get_texture_id() const { return textureID; }

material model_part::get_material() const { return m_material; }
...
void model::add_range(std::vector<float> vertices) {
    for (int dim = 0; dim < 3; ++dim) {
        for (size_t j = dim; j < vertices.size(); j += 3) {
            ...
            maxRanges[2 * dim + 1] =
                std::max(vertices[j], maxRanges[2 * dim + 1]);
        }
    }
}
...
void model::add_model_part(model_part part) { parts.push_back(part); }

const std::vector<model_part>& model::get_model_parts() { return parts; }
}  // namespace ore
